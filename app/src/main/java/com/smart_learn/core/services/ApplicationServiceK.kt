package com.smart_learn.core.services

import com.smart_learn.presenter.view_models.ActivityViewModelUtilitiesK

/**
 * This class should act as a container class for the activityServices classes through which
 * this classes will have centralized access to all non-activity services
 * from the entire program.
 *
 */
class ApplicationServiceK(private val activityViewModelUtilitiesK: ActivityViewModelUtilitiesK<*, *>) {

    var lessonServiceK: LessonServiceK = LessonServiceK(activityViewModelUtilitiesK)

}